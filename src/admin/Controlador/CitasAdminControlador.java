...
public class CitasAdminControlador implements ActionListener {
    ...
    private void sugerirProximaHora(JDAgendarCitaVista modal) {
        
        String sql = "SELECT MAX(fecha_hora) as ultima_cita FROM citas WHERE CAST(fecha_hora AS DATE) >= CURRENT_DATE AND estado_cita = 'PROGRAMADA'";
        try (Connection con = conexion.getConexion(); 
             PreparedStatement ps = con.prepareStatement(sql); 
             ResultSet rs = ps.executeQuery()) {
             
            if (rs.next() && rs.getTimestamp("ultima_cita") != null) {
                Timestamp ultima = rs.getTimestamp("ultima_cita");
                long finUltimaCitaMilis = ultima.getTime() + (20 * 60 * 1000); 
                long ahoraMilis = System.currentTimeMillis();
                if (finUltimaCitaMilis > ahoraMilis) {
                    Date fechaSugerida = new Date(finUltimaCitaMilis);
                    
                    modal.txtFecha.setText(new SimpleDateFormat("yyyy-MM-dd").format(fechaSugerida));
                    modal.spnHora.setValue(fechaSugerida);
                    return;
                }
            }
        } catch (Exception e) {
            System.err.println("Error calculando próxima hora sugerida: " + e.getMessage());
        }

        modal.txtFecha.setText(new SimpleDateFormat("yyyy-MM-dd").format(new Date()));
        modal.spnHora.setValue(new Date());
    }
    ...
    private void cargarDatosTabla() {
        ...
        sql += "ORDER BY c.fecha_hora ASC, c.id_cita ASC LIMIT ? OFFSET ?";
    ...
    private void editarCitaFila() {
        ...
        if (fila != -1) {
            ...
            modalEditar.btnGuardarCambios.addActionListener(ev -> {
                ...
                try (Connection con = conexion.getConexion()) {
                    String sqlValidacion = "SELECT fecha_hora FROM citas WHERE id_veterinario = ? AND id_cita != ? AND estado_cita = 'PROGRAMADA' " +
                                           "AND fecha_hora >= (CAST(? AS TIMESTAMP) - INTERVAL '19 minutes') " +
                                           "AND fecha_hora <= (CAST(? AS TIMESTAMP) + INTERVAL '19 minutes')";
                    try (PreparedStatement psVal = con.prepareStatement(sqlValidacion)) {
                        psVal.setInt(1, nuevoIdVeterinario);
                        psVal.setInt(2, idCita);
                        psVal.setTimestamp(3, Timestamp.valueOf(fechaHoraFinal));
                        psVal.setTimestamp(4, Timestamp.valueOf(fechaHoraFinal));
                        try (ResultSet rsVal = psVal.executeQuery()) {
                            if (rsVal.next()) {
                                String horaOcupada = new SimpleDateFormat("HH:mm").format(rsVal.getTimestamp("fecha_hora"));
                                JOptionPane.showMessageDialog(modalEditar, 
                                    "El médico ya tiene otra cita programada a las " + horaOcupada + ".\n" +
                                    "Por favor elija una hora diferente.", "Horario Ocupado", JOptionPane.WARNING_MESSAGE);
                                return;
                            }
                        }
                    }
    ...
    private void insertarNuevaCita(JDAgendarCitaVista vistaCrear) {
        ...
        try {
            ...
            try (Connection con = conexion.getConexion()) {
                ...
                String sqlValidacion = "SELECT fecha_hora FROM citas WHERE id_veterinario = ? AND estado_cita = 'PROGRAMADA' " +
                                       "AND fecha_hora >= (CAST(? AS TIMESTAMP) - INTERVAL '19 minutes') " +
                                       "AND fecha_hora <= (CAST(? AS TIMESTAMP) + INTERVAL '19 minutes')";
                try (PreparedStatement psVal = con.prepareStatement(sqlValidacion)) {
                    psVal.setInt(1, idVeterinario);
                    psVal.setTimestamp(2, fechaHora);
                    psVal.setTimestamp(3, fechaHora);
                    try (ResultSet rsVal = psVal.executeQuery()) {
                        if (rsVal.next()) {
                            String horaOcupada = new SimpleDateFormat("HH:mm").format(rsVal.getTimestamp("fecha_hora"));
                            JOptionPane.showMessageDialog(vistaCrear, 
                                "El médico ya tiene una cita programada a las " + horaOcupada + ".\n" +
                                "Por favor, asigne una hora con al menos 20 minutos de diferencia.", 
    ...
    private void limpiarCampos(JDAgendarCitaVista vistaCrear){
        ...
        if (vistaCrear.cbMascotasClientes.getItemCount() > 0) vistaCrear.cbMascotasClientes.setSelectedIndex(0);
        if (vistaCrear.cbVeterinarios.getItemCount() > 0) vistaCrear.cbVeterinarios.setSelectedIndex(0);
        
        sugerirProximaHora(vistaCrear);
    }
}